...
export default function CategoryContent({ products, categoryTitle }: CategoryContentProps) {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 lg:gap-10">
      ...
      <div className="md:hidden flex justify-between items-center bg-white/60 backdrop-blur-md p-4 rounded-2xl border border-black/5 mb-2">
        <button
          onClick={() => setIsMobileFilterOpen(true)}
          className="flex items-center gap-2 text-xs font-mono font-bold uppercase tracking-widest bg-[#527661] text-white px-5 py-2.5 rounded-full shadow-sm"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
            <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 6h9.75M10.5 6a1.5 1.5 0 1 1-3 0m3 0a1.5 1.5 0 1 0-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m-9.75 0h9.75" />
          </svg>
          Filter Products {hasActiveFilters && `(${selectedColors.length + selectedPriceRanges.length + selectedSizes.length})`}
        </button>
      ...
      {isMobileFilterOpen && (
        <div className="fixed inset-0 z-50 md:hidden flex justify-end">
          ...
          />
          <div className="relative w-4/5 max-w-xs bg-white h-full shadow-2xl p-6 overflow-y-auto z-10 flex flex-col justify-between">
            <div>
            ...
            <button
              onClick={() => setIsMobileFilterOpen(false)}
              className="w-full bg-[#527661] text-white py-3 rounded-full font-bold text-xs uppercase tracking-widest mt-6"
            >
              Apply Filters
            </button>
          </div>
      ...
      <aside className="hidden md:block w-[220px] lg:w-[240px] flex-shrink-0 sticky top-28 self-start max-h-[calc(100vh-140px)] overflow-y-auto scrollbar-thin pr-2">
        {renderFilterSections()}
      </aside>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between mb-8 flex-shrink-0">
          <p className="text-xs font-mono tracking-wider text-black/40 uppercase hidden md:block">
          ...
          <div className="flex items-center gap-3 ml-auto">
            ...
            <select
              ...
              className="text-xs sm:text-sm bg-white border border-black/10 rounded-lg px-4 py-2 text-black/70 focus:outline-none focus:border-[#527661] transition-colors cursor-pointer appearance-none pr-8"
              style={{
              ...
              {filteredProducts.map((product) => (
                <div key={product.id} className="group flex flex-col justify-between">
                  ...
                  <button
                    ...
                    className="w-full mt-4 py-2.5 border border-black/15 rounded-lg text-[11px] font-bold uppercase tracking-widest text-black/70 hover:bg-[#527661] hover:text-white hover:border-[#527661] transition-all duration-300"
                  >
                    Add To Cart
                  </button>
                </div>
